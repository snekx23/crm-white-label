...
const navItems = [
  ...
  { href: "/disparos", label: "Disparo em Massa", icon: Megaphone },
];
...
export function Sidebar({
  ...
  const filteredNavItems = isVendedor
    ? [
        { href: "/leads", label: "Meus Clientes", icon: Users },
        { href: "/kanban", label: "Meu Funil", icon: KanbanSquare },
        { href: "/agenda", label: "Agenda de Shows", icon: CalendarDays },
      ]
    : navItems;

  const filteredSecondaryItems = isVendedor ? [] : secondaryItems;
  ...
  return (
    <aside className="flex w-64 shrink-0 flex-col border-r border-border bg-card shadow-[inset_-1px_0_0_hsl(var(--foreground)/0.04)] dark:border-border/50 dark:bg-card/75">
      ...
      {!isVendedor ? (
        ...
        <div className="mx-3 mt-3 flex items-center gap-2.5 rounded-lg border border-border/60 bg-background/20 p-2.5 text-left">
          <div className="grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-md bg-brand/15 font-display text-xs font-semibold text-brand ring-1 ring-border/50">
            {tenantLogoUrl ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={tenantLogoUrl} alt={tenantName} className="h-full w-full object-cover" />
          ...
          <div className="flex-1 overflow-hidden">
            <p className="truncate text-sm font-semibold leading-tight">{tenantName}</p>
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Painel Vendedor</p>
          </div>
        </div>
      )}
